...
const SuperFranchiseList = () => {
  ...
  const approverName = useMemo(() => {
    const f = (user?.fname || user?.firstName || "").trim();
    const l = (user?.lname || user?.lastName || "").trim();
    const nm = [f, l].filter(Boolean).join(" ").trim();
    return nm || user?.email || "Superadmin";
  }, [user]);
                ...
                {loading ? (
                  ...
                  rows.map((r, index) => (
                    ...
                    >
                      ...
                      <td style={styles.td}>
                        {fmtMoney(r.investmentBudget)}
                      </td>
                      <td style={styles.td}>{r.timeline}</td>
                      <td style={styles.td}>
                        <StatusPill v={r.status || "pending"} />
                      </td>
                      <td
                        style={{
                          ...styles.td,
