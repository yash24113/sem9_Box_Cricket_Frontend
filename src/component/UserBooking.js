...
const UserBooking = () => {
  ...
  /* ---------- Load logged-in user once (no loop) ---------- */
  useEffect(() => {
    try {
      const raw = localStorage.getItem("loggedInUser");
      if (raw) {
        const parsed = JSON.parse(raw);
        const user = parsed?.data || parsed || null;
  ...
  /* ---------- Fetch bookings (depends on page + user, not on every render) ---------- */
  ...
  useEffect(() => {
    if (!loggedInUser) {
      // Wait until user is resolved by first effect
      return;
    }
    if (!loggedInUser._id) {
      // Not logged in -> stop loader, no loop
      setLoading(false);
      setBookings([]);
      setTotalPages(1);
      return;
                    ...
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                      <Grid item xs={12} md={6}>
                        <Typography variant="body1">
                          ...
                          />
                          <strong>Time:</strong>{" "}
                          {booking.slot_id?.slot_start_time} -{" "}
                          {booking.slot_id?.slot_end_time}
                        </Typography>
                        <Typography variant="body1">
                          <CalendarTodayIcon
                            color="action"
                    ...
                    >
                      ...
                      >
                        <Button
                          ...
                          onClick={() =>
                            handleDownloadTicket(booking._id)
                          }
                        >
                          Ticket
                        </Button>
        ...
        <DialogContent>
          <Box sx={{ mt: 2, mb: 3 }}>
            ...
            >
              ...
              {selectedBooking?.area_id?.area_name ||
                "this venue"}
              ?
            </Typography>
            <Box
