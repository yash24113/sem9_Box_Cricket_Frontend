...
const BookingForm = () => {
  ...
  const getInitialContext = () => {
    ...
    try {
      stored =
        JSON.parse(
          localStorage.getItem("pendingBookingContext") || "null"
        ) || {};
    } catch {
      stored = {};
    }
    ...
    const pick = (key, altKeys = []) => {
      ...
      const fromQuery =
        search.get(key) ||
        altKeys.map((k) => search.get(k)).find((v) => v);
  ...
  const formatTimeLeft = (seconds) => {
    ...
    const sec = String(seconds % 60).padStart(
      2,
      "0"
    );
    return `${min}:${sec}`;
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      setServerError(
        error?.message ||
          "Stripe payment or booking failed"
      );
      toast.error(
        error?.message ||
          "Stripe payment or booking failed"
