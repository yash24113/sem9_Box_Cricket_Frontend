...
const AreaWiseSlot = () => {
    ...
    const fetchSlots = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/userapi/viewAreaWiseSlot?area=${encodeURIComponent(areaName)}`
        );
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/userapi/viewBooking");
        const apiBookings = response.data?.data || [];

        const matchingBookings = apiBookings.filter(
          (booking) => dayjs(booking.date).format("YYYY-MM-DD") === selectedDate
        );
  ...
  useEffect(() => {
    ...
    if (priceFilter) {
      filtered = filtered.filter((slot) =>
        slot.price.toString().includes(priceFilter)
      );
    }
    ...
    if (selectedDate === dayjs().format("YYYY-MM-DD")) {
      ...
      filtered = filtered.filter((slot) => {
        ...
        const slotEndTime = dayjs().hour(endHour).minute(endMin);
        return slotEndTime.isAfter(now);
      });
    }

    filtered.sort((a, b) => {
      const [aHour, aMinute] = a.slot_start_time.split(":").map(Number);
      const [bHour, bMinute] = b.slot_start_time.split(":").map(Number);
      return aHour !== bHour ? aHour - bHour : aMinute - bMinute;
    });

    setFilteredSlots(filtered);
  }, [slots, bookings, startTimeFilter, endTimeFilter, priceFilter, selectedDate]);
  ...
  return (
    <div style={styles.container}>
      <h2>
        Available Slots for{" "}
        <span style={{ color: "#007bff" }}>{areaName || "Unknown Area"}</span>
      </h2>
      ...
      {loading ? (
        ...
        <div>
          ...
          <button style={styles.tryAnotherButton} onClick={() => navigate("/arealist")}>
            Back to Area List
          </button>
        </div>
        ...
        <div style={styles.cardContainer}>
          {filteredSlots.map((slot, index) => {
            const isBooked = bookings.some((booking) => booking.slot_id === slot._id);
            return (
              <div
                key={slot._id || index}
                style={{
        ...
        <div>
          <p style={styles.noData}>No slots available for this area on selected date.</p>
          <button style={styles.tryAnotherButton} onClick={() => navigate("/arealist")}>
            Try Another Area
          </button>
        </div>
      ...
      <button style={styles.backButton} onClick={() => navigate("/arealist")}>
        Back to Area List
      </button>
    </div>
  );
